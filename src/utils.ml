...
let find_process_pid regex =
  ...
  ListLabels.find_map processes ~f:(fun (pid, cmd) -> if Str.string_match regex cmd 0 then Some pid else None)
  |> Lwt.return
...
let get_or_timeout ?(timeout = 1.0) uri =
  ...
  let timeout = Lwt_unix.sleep timeout >>= fun () -> Lwt.return `Timeout in
  let get =
    try%lwt Cohttp_lwt_unix.Client.get uri >>= fun response -> Lwt.return (`Response response)
    with exn -> Lwt.return (`Exception exn)
  in
