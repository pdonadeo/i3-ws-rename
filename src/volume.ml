module PulseAudioInterface = struct
  ...
  let deallocate ud =
    if not (is_null ud.context) then Pa.context_unref ud.context;
    Pa.signal_free ud.unix_signal_ev_src;
    Pa.glib_mainloop_free ud.mainloop

  let quit ret_code ud = Lwt.wakeup_later ud.resolver ret_code
  ...
  let subscribe_callback _c type_ _idx ud_c =
    ...
    let facility =
      Pa.Subscription_event_type_t.to_int type_
      land Pa.Subscription_event_type_t.to_int `PA_SUBSCRIPTION_EVENT_FACILITY_MASK
      |> Pa.Subscription_event_type_t.of_int
    in
    match facility with
  ...
  let rec context_state_callback context ud_c =
    ...
    | `PA_CONTEXT_FAILED -> begin connect_to_pulse ud_c |> ignore end
    | `PA_CONTEXT_TERMINATED -> begin quit 0 ud end
...
class ['message, 'a] modulo instance_name status_pipe color color_muted sep : ['a] Lwt_module.modulo =
  object (self)
    ...
    method! private loop () =
      ...
      | Data_available t -> begin
        ...
        | volume, mute -> begin
          ...
          | Some (old_vol, old_mute) -> begin
            if volume <> old_vol || mute <> old_mute
            then begin
      ...
      | Pipe_closed | Nothing_available -> Lwt.return ()

    method! run () : unit Lwt.t =
      Utils.detach_promise self#loop "Volume.modulo #loop method";
      Utils.detach_promise self#main "Volume.modulo #main method";
    ...
    method! json () =
      ...
      let full_text, short_text, color =
        match state with
        | None -> begin (note ^ "?", "", color_muted) end
