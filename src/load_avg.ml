  ...
  object (self)
    ...
    method! json () =
      ...
      let color, full_text, short_text =
        ...
        | None, _ -> color_good, "", ""
        | Some l, lp when lp <= 85. -> color_good, spf "%s %0.2f" icon l, ""
        | Some l, lp when lp <= 85. -> color_degraded, spf "%s %0.2f" icon l, spf "%s %0.2f" icon l
        | Some l, lp when lp <= 95. -> color_bad, spf "%s %0.2f" icon l, spf "%s %0.2f" icon l
