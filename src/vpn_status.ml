...
class ['a] modulo instance_name status_pipe color sep : ['a] Lwt_module.modulo =
  object (self)
    ...
    method private is_vpn manager path_str =
      ...
      let vpn_prop =
        OBus_property.make
          Nm_interfaces.Org_freedesktop_NetworkManager_Connection_Active.p_Vpn
          conn_proxy in
    ...
    method private update_active_vpn manager new_list =
      let%lwt new_set = Lwt_list.fold_left_s (fun set path ->
        let%lwt (conn_id, vpn) = self#is_vpn manager path in
        if vpn
        then BatSet.String.add conn_id set |> Lwt.return
        else Lwt.return set
      ) BatSet.String.empty new_list in
    ...
    method private network_manager () =
      ...
      let nm_proxy = OBus_proxy.make
        ~peer:(Nm_manager.to_peer manager)
        ~path:["org"; "freedesktop"; "NetworkManager"] in

      let sig_desc =
        OBus_signal.make
          Nm_interfaces.Org_freedesktop_NetworkManager.s_PropertiesChanged
          nm_proxy in
          ...
          (fun properties ->
              Lwt_list.iter_s (fun (k, v) ->
                if k = "ActiveConnections" then (
                  let active_now =
                    C.cast_single (C.array C.basic_object_path) v
                    |> List.map OBus_path.to_string in
    ...
    method! run () : unit Lwt.t =
      Lwt.async self#network_manager;
    ...
    method! json () =
      ...
      let full_text, short_text =
        if List.length vpns <> 0
        then String.concat " " vpn_icons, icon
        else "", "" in
