...
let rainbow_radar ~state _time =
  ...
  *)
  let l_out = (((1./.24.*.(0.5-.(float_of_int count))) -. (floor (1./.24.*.(0.5-.(float_of_int count))))) ** 3.) /. 2. in
  let l_in  = (((1./.24.*.(0.5-.(float_of_int (count + 2)))) -. (floor (1./.24.*.(0.5-.(float_of_int (count + 2)))))) ** 3.) /. 2. in
...
let default_status = {
  current_animation = 0;
  show_animation_name = false;
  show_started = 0.0;
}

let get_current_name s =
  (List.nth animations s.current_animation).name

let toggle_show s = { s with
  show_animation_name = not s.show_animation_name;
  show_started = Unix.gettimeofday ();
}

let change_animation s =
  let how_many = List.length animations in
  { s with
    current_animation = (s.current_animation + 1) mod how_many;
    show_started = Unix.gettimeofday ();
  }
