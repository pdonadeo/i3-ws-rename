...
let rec read_click_event () =
  ...
  then read_click_event ()
    ...
    let msg = if BatString.starts_with l "," then BatString.strip ~chars:"," l else l in
...
let rec read_clicks_loop (running_instances : [`r | `w] Lwt_module.modulo StringTuple2Map.t) () =
  ...
  let maybe_mod = try Some (StringTuple2Map.find (name, instance) running_instances) with Not_found -> None in
...
let _ =
  Lwt_unix.on_signal Sys.sigusr1 (fun _ ->
      ...
      Utils.detach_promise
        (fun () ->
          let%lwt _ = Lwt_pipe.write pipe `Stop_output in
          Lwt.return_unit)
        "I3_status sigusr1 handler")

let _ =
  Lwt_unix.on_signal Sys.sigusr2 (fun _ ->
...
let entry_point state_fname shutdown () =
  ...
  let%lwt running_instances =
    Lwt_list.fold_left_s
      ...
      StringTuple2Map.empty
      modules
  in

  Utils.detach_promise (read_clicks_loop running_instances) "I3_status.entry_point running read_clicks_loop";
  ...
  let rec loop ?(enable_output = true) () =
    ...
    then begin
      ...
      let%lwt state =
        Lwt_list.fold_left_s
          (fun serialized modulo ->
            let%lwt state = modulo#dump_state () in
            let serialized =
              { StringTuple2Map_Json.key = (modulo#name, modulo#instance); StringTuple2Map_Json.data = state }
              :: serialized
            in
      ...
      | Pipe_closed | Nothing_available -> Lwt.return_unit
    end
  in

  Utils.detach_promise loop "I3_status.entry_point running loop";
  completed
